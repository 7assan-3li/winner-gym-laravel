<div class="wg-inventory-page wg-sales-page" id="sales-pos-top" dir="rtl"
    ...
    <article class="wg-inv-card wg-sales-history">
        ...
        @forelse($sales as $sale)
            ...
            <tr><td><strong class="wg-code">{{ $sale->sale_number }}</strong></td><td><strong>{{ $customer }}</strong>@if($sale->member)<small class="wg-subline">{{ $sale->member->membership_code }}</small>@endif</td><td><strong>{{ $sale->items->count() }} منتج</strong><small class="wg-subline">{{ number_format($units) }} وحدة</small></td><td class="wg-inv-money"><strong>{{ \App\Support\NumberFormatter::money($sale->total_amount) }}</strong> <small>{{ $sale->currency }}</small>@if((float)$sale->discount_amount>0)<small class="wg-subline is-orange">خصم {{ \App\Support\NumberFormatter::money($sale->discount_amount) }}</small>@endif</td><td>{{ $sale->payment_method==='transfer' ? 'تحويل' : 'نقدي' }}@if($sale->transfer_reference)<small class="wg-subline">{{ $sale->transfer_reference }}</small>@endif@if($sale->proof_path)<a class="wg-subline" href="{{ route('inventory.sales.proof', $sale) }}" target="_blank" rel="noopener">عرض السند</a>@endif</td><td>{{ optional($sale->sold_at)->timezone('Asia/Aden')->format('Y-m-d h:i A') }}</td><td>@if($sale->status==='completed')<span class="wg-inv-status is-ok">مكتمل</span>@else<span class="wg-inv-status is-inactive">ملغي</span>@endif</td><td><div class="wg-inv-row-actions"><a href="{{ route('inventory.sales.receipt', $sale) }}" target="_blank" class="wg-pos-print-action" title="طباعة الإيصال"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor"><path d="M7 8V3h10v5M7 17h10v4H7z"/><path d="M5 17H3v-7h18v7h-2"/></svg></a>@if($canCancel && $sale->status==='completed')<button type="button" wire:click="startCancel({{ $sale->id }})" class="is-danger" title="إلغاء البيع وإرجاع المخزون"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor"><path d="m6 6 12 12M18 6 6 18"/></svg></button>@endif</div></td></tr>
